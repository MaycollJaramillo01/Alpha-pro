...
if (!function_exists('vdHeroTitleHtml')) {
  function vdHeroTitleHtml($label)
  {
    ...
    if (!$parts || count($parts) === 0) return 'Project';
    if (count($parts) === 1) return htmlspecialchars($parts[0], ENT_QUOTES, 'UTF-8');
    $first = htmlspecialchars(array_shift($parts), ENT_QUOTES, 'UTF-8');
    $rest = htmlspecialchars(implode(' ', $parts), ENT_QUOTES, 'UTF-8');
...
.hero-displace__overlay {
  ...
  z-index: 4;
  background:
    radial-gradient(circle at 84% 18%, rgba(17, 23, 33, 0.12), transparent 34%),
    linear-gradient(96deg, rgba(11, 16, 23, 0.88) 8%, rgba(20, 28, 39, 0.66) 44%, rgba(15, 20, 29, 0.83) 100%);
}
...
(function () {
  ...
  function preloadMedia() {
    const mediaElements = Array.from(root.querySelectorAll('.hero-displace__asset'));
    const mediaReadyPromises = mediaElements.map(function (media) {
    ...
    return Promise.all(mediaReadyPromises);
  }
